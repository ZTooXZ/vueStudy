...
export default {
  ...
  methods: {
    ...
    //删除指定id的todo //_占位，因为pubsub方法回调函数的值有两个 一个是 msgName ，id  但是msgName不使用
    deleteTodo(_,id){
...
/*     this.$bus.$on('changeTodo',this.changeTodo)
    ...
    this.pubId1 = pubsub.subscribe('changeTodo',this.changeTodo)
    this.pubId2 = pubsub.subscribe('deleteTodo',this.deleteTodo)
  },
  beforeDestroy() {
/*     this.$bus.$off('changeTodo')
    this.$bus.$off('deleteTodo') */
    pubsub.unsubscribe(this.pubId1,this.pubId2)
  },
}
</script>
